...
firstHalf x
  | even $ length x = take takeNum x
  | otherwise = take (takeNum + 1) x
  where
    takeNum = length x `div` 2
...
joinToLength ln xs = [x ++ y | x <- xs, y <- xs , length (x ++ y) == ln]
...
interpreterHelper (_, _) [] res = reverse res
interpreterHelper (x, y) (c : cs) res = case c of
  "up" -> interpreterHelper (x, y + 1) cs res
  "down" -> interpreterHelper (x, y - 1) cs res
  "left" -> interpreterHelper (x - 1, y) cs res
  "right" -> interpreterHelper (x + 1, y) cs res
  "printX" -> interpreterHelper (x, y) cs (show x:res)
  "printY" -> interpreterHelper (x, y) cs (show y:res)
  _ -> interpreterHelper (x, y) [] res
