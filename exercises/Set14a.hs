...
greetText name = T.concat [T.pack "Hello, ", ellipsis 15 name, T.pack "!"]
  where
    ellipsis:: Int -> T.Text -> T.Text
    ellipsis len t
      | T.length t <= len = t
      | otherwise = T.append (T.take len t) (T.pack "...")
...
shout :: T.Text -> T.Text
-- shout t = T.unwords $ map (\(i,v) -> if i == 0 || even i then T.toUpper v else v ) (zip [0..] (T.words t))
shout t = T.unwords $ zipWith (\ i v -> (if i == 0 || even i then T.toUpper v else v)) [0..] (T.words t)
...
longestRepeat t = foldl (\acc x -> max (T.length x) acc) 0 (T.group t)
...
takeStrict n lt = TL.toStrict $ TL.take n lt
