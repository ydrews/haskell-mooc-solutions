...
outputInInput input output = forAll (elements output) (\(x, n) -> n === occurs x input)
  where
    occurs x = foldr (\y acc -> if x == y then acc + 1 else acc ) 0
...
frequenciesProp freq input =
  wrapTest sumIsLength input .&&.
  wrapTest inputInOutput input .&&.
  wrapTest outputInInput input
  where
    wrapTest testFn (NonEmpty input') = testFn input' (freq input')
...
genList = do
  len <- choose (3, 5)
  vector <- vectorOf len (elements [0..10])
  return (sort vector)
...
instance Arbitrary Arg where
  arbitrary = do
    let num = elements (map Number [0..10])
        var = elements (map Variable ['a','b','c','x','y','z'])
    oneof [num, var]
...
instance Arbitrary Expression where
  arbitrary = do
    ...
    let plusGen  = return (Plus arg1 arg2)
        minusGen = return (Minus arg1 arg2)
    oneof [plusGen, minusGen]
