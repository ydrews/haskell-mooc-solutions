...
allEqual [] = True
allEqual [x] = True
allEqual (x:y:xs) = x == y && allEqual (x:xs)
...
longest :: Ord a => [[a]] -> [a]
longest xs = head (filter (\x -> head x == minHead) allMax)
  where
    maxItemLength = maximum $ map length xs
    allMax = filter (\x -> length x == maxItemLength) xs
    minHead = minimum $ map head allMax
...
incrementKey _ [] = []
incrementKey k ((key,value):xs)
  | k == key = (key, value + 1) : incrementKey k xs
  | otherwise = (key, value) : incrementKey k xs
...
freqs = foldr (Map.alter plusOne) Map.empty
  where
...
transfer from to amount bank
  | amount < 0 = bank
  | not isFromMember || not isToMember = bank
  | maybeLess (Map.lookup from bank) amount = bank
  | maybeSum (Map.lookup from bank) (Map.lookup to bank) < 0 = bank
  | otherwise = Map.insertWith (+) to amount (Map.insertWith discard from amount bank)
  where
    isFromMember = Map.member from bank
    isToMember = Map.member to bank

    discard a b = b - a

    maybeSum (Just a) (Just b) = a + b
    maybeSum (Just a) Nothing = a
    maybeSum Nothing (Just b) = b
    maybeSum Nothing Nothing = 0

    maybeLess (Just b) a = b < a
    maybeLess Nothing a = True
...
maxIndex arr = foldr (\x acc -> if arr ! x >= arr ! acc then x else acc) headIndex indexes
  where
    indexes = indices arr
    headIndex = head indexes
