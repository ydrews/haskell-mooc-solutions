...
instance Functor SL where
  -- implement fmap
  fmap f (SL m) = SL (\st -> let (a, s, l) = m st in (f a, s, l))
...
adder :: IORef Int -> IO()
adder ref = modifyIORef ref (+1)

mkCounter :: IO (IO (), IO Int)
mkCounter = do
  ref <- newIORef 0
  return (adder ref, getter ref)
