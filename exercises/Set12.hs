...
fmap2 fun = fmap (fmap fun)

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 fun = fmap (fmap2 fun)
...
instance Foldable List where
  ...
  foldr f initial (LNode a Empty) = f a initial
  foldr f initial (LNode a list) = f a (foldr f initial list)
...
instance Foldable TwoList where
  ...
  foldr f initial (TwoNode a b TwoEmpty) = f a $ f b initial
  foldr f initial (TwoNode a b list) = f a . f b $ foldr f initial list
