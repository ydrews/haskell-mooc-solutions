...
hSelectLines h nums = do
  lns <- hFetchLines h
  return (map snd . filter filterHelper $ zip [1 ..] lns)
  where
    filterHelper x = fst x `elem` nums
...
extractFst :: (Bool, String, st) -> Bool
extractFst (a,_,_) = a

extractSnd :: (Bool, String, st) -> String
extractSnd (_,a,_) = a

extractThrd :: (Bool, String, st) -> st
extractThrd (_,_,a) = a

interact' :: ((String,st) -> (Bool,String,st)) -> st -> IO st
interact' f state = do
  ln <- getLine
  putStrLn (extractSnd $ f (ln, state))
  if extractFst $ f (ln,state)
    then do
      interact' f (extractThrd $ f (ln, state))
    else do
      return (extractThrd $ f (ln, state))
