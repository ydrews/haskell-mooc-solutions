...
largestHelper item [] = [item]
largestHelper item acc
  | maximum acc == item = item : acc
  | maximum acc < item = [item]
  | otherwise = filter (> item) acc
