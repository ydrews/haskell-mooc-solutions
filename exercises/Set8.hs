...
dotAndLine = Picture f
  where f (Coord 3 4) = white  -- white dot at (3,4)
        f (Coord _ 8) = pink   -- white line at y = 8
        f (Coord _ _) = black  -- rest of the picture is black
------------------------------------------------------------------------------
...
combine f (Picture pf1) (Picture pf2) = Picture ff
  where
    ff coord = f (pf1 coord) (pf2 coord)
...
rectangle x0 y0 w h = Shape f
  where f (Coord x y) = x < (x0 + w) && y < (y0 + h) && x >= x0 && y >= y0
------------------------------------------------------------------------------
...
union (Shape sf1) (Shape sf2) = Shape sf3
  where sf3 coord = sf1 coord || sf2 coord

cut :: Shape -> Shape -> Shape
cut (Shape sf1) (Shape sf2) = Shape sf3
  where sf3 coord = sf1 coord && not (sf2 coord)
------------------------------------------------------------------------------
...
paintSolid color (Shape sf) (Picture pf) = Picture ff
  where
    ff coord
      | sf coord = color
      | otherwise = pf coord
...
paint (Picture patf) (Shape sf) (Picture pf) = Picture ff
  where
    ff coord
      | sf coord = patf coord
      | otherwise = pf coord
...
instance Transform Fill where
  apply (Fill c) (Picture f) = Picture ff
    where ff _ = c
...
instance Transform Flip where
  apply FlipX (Picture f) = Picture ff
    where
      ff (Coord x y) = f (Coord (-x) y)
  apply FlipY (Picture f) = Picture ff
    where
      ff (Coord x y) = f (Coord x (-y))
  apply FlipXY (Picture f) = Picture ff
    where
      ff (Coord x y) = f (Coord y x)
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture ff
    where
      ff (Coord x y) = blendMultiple (map (\(x, y) -> f (Coord x y)) [(x, y), (x - 1, y), (x, y - 1), (x + 1, y), (x, y + 1)])
      blendMultiple :: [Color] -> Color
      blendMultiple cs = Color (blendRed cs) (blendGreen cs) (blendBlue cs)
      blendRed cs = div (sum $ map getRed cs) 5
      blendGreen cs = div (sum $ map getGreen cs) 5
      blendBlue cs = div (sum $ map getBlue cs) 5
------------------------------------------------------------------------------
